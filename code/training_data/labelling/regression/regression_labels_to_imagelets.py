...
def batch_extract_labels(city_name, in_dir, selected_var, \
	...
	try:
		label = \
			gp.read_file("preprocessed/regression_labels/label_shapes/"+\
				city_name+"/"+selected_var+"_epsg_32632.shp")
	except:
		return None

	max_label = label["class"].max()
	min_label = label["class"].min()
	...
	for root, subdirs, fl in os.walk(in_dir):
		for f in fl:
			if f.endswith(".shp"):
				...
				if create_high_low_examples:
					if one_label["label_" + selected_var] in [min_label,max_label]:
						jName = f.replace(".shp", ".jpg")
						jPath = jpg_dir
						src = os.path.join(jPath, jName)
						if one_label["label_" + selected_var] == max_label:
							dst = os.path.join(high_examples_folder, jName)
						elif one_label["label_" + selected_var] == min_label:
...
def label_one_city(city_name = "milano"):
	...
	out_file = "preprocessed/regression_labels/" + city_name.lower() + "_imagelet_labels.csv"

	examples_folder = "preprocessed/regression_high_low_examples/"+city_name+"/"
	jpg_dir = \
